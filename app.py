...
# --- 1. OPTIMIZED KML TO GEOJSON PARSER ---
...
if uploaded_csv is not None:
    ...
    # 2. Advanced Calculations (Global)
    # NEW FORMULA: Presence=1, Crop=2.5, House=5, Injury=20
    ...
    with st.sidebar:
        ...
        # Filter Previous Period Data (for Trend comparison)
        ...
        if sel_div != 'All': 
            ...
            df_prev = df_prev[df_prev['Division'] == sel_div] # Apply same filter to previous
    ...
    # --- KPI METRICS ---
    # Helper for Metrics with Delta
    def get_metric_delta(current_val, prev_val):
    ...
    # 4. Top Conflict Hotspot (Beat)
    # Group by Beat to find highest severity score
    if not df.empty:
    ...
    with st.expander("ℹ️ Indicator Methodology"):
        st.markdown("""
        * **Conflict Severity Score:** Weighted Index. *Score = (Presence × 1) + (Crop × 2.5) + (House × 5) + (Injury × 20)*.
    ...
    with c_legend:
        ...
        active_beats = set(df['Beat'].unique())
        active_beats_clean = {b.strip().title() for b in active_beats}
        all_beats_clean = {b.strip().title() for b in all_loaded_beats}
        missing_beats = list(all_beats_clean - active_beats_clean)
        coverage_pct = len(active_beats_clean) / len(all_beats_clean) * 100 if all_beats_clean else 0
    ...
    with r1c2:
        ...
        if not df.empty:
            # Change to simple bar chart for Hours (0-23)
